...
if [ ${USER_CONTENT_DB_TYPE} = "mongodb" ]
  then
    wait_database $MONGOS1_HOST $MONGOS1_PORT $USER_CONTENT_DB_TYPE
fi


if [ ${USER_CONTENT_DB_TYPE} = "mongodb" ]
  then
    wait_database $MONGOS2_HOST $MONGOS2_PORT $USER_CONTENT_DB_TYPE
fi
