...
from ...models.film_review import FilmReview
from ...models.response_models import FilmReviewListResponse


class MongoFilmReviewList(MongoPageNavigationTemplateList):
    ...
    def __init__(
        ...
        self.collection: AsyncIOMotorCollection = self.database[CollectionName.FilmReview.value]
        self.model = FilmReview
        self.response_model = FilmReviewListResponse

    def _get_query(self) -> dict:
        """Метод получает запрос."""
        try:
            return dict(film_id=self.filter['film_id'])
        except KeyError:
            raise HTTPException(status_code=HTTPStatus.BAD_REQUEST, detail='Не задан фильтр по фильму.')
